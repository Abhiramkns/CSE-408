...
def start(studentId):
    ...
    return gr.update(visible=False), gr.update(value=selected_image, visible=True), gr.update(visible=True), gr.update(visible=True), gr.update(visible=True), gr.update(visible=False)
...
def got_to_nextexe(answer3, answer4):
    ...
    student.count += 1
    if student.count == 20:
      return  gr.update(visible=True), gr.update(value=selected_image, visible=False), gr.update(visible=False), gr.update(visible=False), gr.update(visible=False), gr.update(visible=False), gr.update(visible=False), gr.update(visible=False)
    
    selected_image = get_image()
    return gr.update(visible=False), gr.update(value=selected_image, visible=True), gr.update(visible=True), gr.update(visible=True), gr.update(visible=False), gr.update(visible=False), gr.update(visible=True), gr.update(visible=False)
...
with gr.Blocks() as iface:
    ...
    q4 = gr.Slider(label="Q4", maximum=9)
    q5 = gr.Textbox(label="Q5")
    ...
    start_btn.click(start, inputs=studentId, outputs=[studentId, img, q2, q3, nextq_btn, start_btn])
    nextq_btn.click(go_to_nextq, inputs=[q2, q3], outputs=[q2, q3, q4, q5, nextq_btn, nextexe_btn])
    nextexe_btn.click(got_to_nextexe, inputs=[q4, q5], outputs=[finish, img, q2, q3, q4, q5, nextq_btn, nextexe_btn])
